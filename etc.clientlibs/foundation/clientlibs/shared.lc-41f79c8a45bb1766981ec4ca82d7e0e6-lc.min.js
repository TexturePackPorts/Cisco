...
_g.shared.HTTP = new function() {
    ...
    return {
        ...
        post: function(a, d, b, c, h, f) {
            ...
            if (h = _g.HTTP.getXhrHook(a, "POST", b))
                a = h.url,
                b = h.params;
            if (void 0 != d)
                return _g.$.ajax({
        ...
        addParameter: function(a, d, b) {
            if (b && b instanceof Array) {
                for (var c = 0; c < b.length; c++)
                    a = _g.HTTP.addParameter(a, d, b[c]);
                return a
            }
            c = -1 == a.indexOf("?") ? "?" : "\x26";
            var e = a.indexOf("#");
        ...
        buildPostResponseFromNode: function(a, d) {
            ...
            for (var c = 0; c < a.childNodes.length; c++) {
                ...
                g.tagName && (g.id && (d.headers[g.id] = g.href ? g.href : g.innerHTML),
        ...
        buildPostResponseFromHTML: function(a) {
            var d = b();
            try {
                void 0 != a.responseText ? a = a.responseText : "string" != typeof a && (a = a.toString());
        ...
        setCookie: function(a, b, c, g, h, f) {
            ...
            if (0 < g) {
                ...
                d = new Date(0);
            document.cookie = encodeURIComponent(a) + "\x3d" + encodeURIComponent(b) + "; " + (0 != g ? "expires\x3d" + d.toGMTString() + "; " : "") + (h ? "domain\x3d" + h + "; " : "") + (c ? "path\x3d" + c : "") + (f ? "; secure" : "");
            return b
        },
        ...
        externalize: function(a, b) {
            if ("undefined" != typeof G_IS_HOOKED && G_IS_HOOKED(a))
                return a;
        ...
        handleForbidden: function(a, b) {
            try {
                return 403 == a[_g.HTTP.HEADER_STATUS.toLowerCase()] ? (Granite.HTTP.handleLoginRedirect(),
                !0) : !1
            } catch (e) {
                return !1
            }
        ...
        reloadHook: function(a) {
            "undefined" != typeof G_RELOAD_HOOK && _g.$.isFunction(G_RELOAD_HOOK) && ("" != CQURLInfo.selectorString && (a = _g.HTTP.addSelector(a, CQURLInfo.selectorString)),
            a = G_RELOAD_HOOK(a) || a);
            return a
        }
    }
...
_g.shared.Util = new function() {
    return {
        ...
        open: function(b, c, a, d) {
            c || (c = window);
            if (b)
                return b = _g.HTTP.reloadHook(b),
                a || (a = ""),
                d || (d = ""),
                c.open(b, a, d)
        },
        htmlEncode: function(b) {
            return b ? String(b).replace(/&/g, "\x26amp;").replace(/>/g, "\x26gt;").replace(/</g, "\x26lt;").replace(/"/g, "\x26quot;") : b
        },
        htmlDecode: function(b) {
            return b ? String(b).replace(/&gt;/g, "\x3e").replace(/&lt;/g, "\x3c").replace(/&quot;/g, '"').replace(/&amp;/g, "\x26") : b
        },
        ellipsis: function(b, c, a) {
            if (b && b.length > c) {
                if (a) {
                    a = b.substr(0, c - 2);
                    var d = Math.max(a.lastIndexOf(" "), a.lastIndexOf("."), a.lastIndexOf("!"), a.lastIndexOf("?"), a.lastIndexOf(";"));
                    return -1 == d || d < c - 15 ? b.substr(0, c - 3) + "..." : a.substr(0, d) + "..."
                }
...
_g.shared.Sling = function() {
    return {
        ...
        getContentPath: function(b, c, a) {
            c.lastIndexOf(".") > c.lastIndexOf("/") && (c = c.substr(0, c.indexOf(".", c.lastIndexOf("/"))));
            if (b)
                if (0 == b.indexOf("/"))
                    c = b;
                else {
...
_g.shared.XSS = new function() {
    return {
        ...
        getXSSRecordPropertyValue: function(b, c, a) {
            var d = "";
            b && c && (d = (d = b.get(this.getXSSPropertyName(c))) ? d : this.getXSSValue(b.get(c)),
            a && !isNaN(a) && (d = _g.Util.ellipsis(d, a, !0)));
            return d
        },
        getXSSTablePropertyValue: function(b, c, a) {
            var d = "";
            b && c && (d = (d = b[this.getXSSPropertyName(c)]) ? d : this.getXSSValue(b[c]),
            a && !isNaN(a) && (d = _g.Util.ellipsis(d, a, !0)));
            return d
        },
        ...
        updatePropertyName: function(b, c) {
            b && c && b[c] && b.xssProtect && !b.xssKeepPropName && (b[c] = this.getXSSPropertyName(b[c]))
        },
        xssPropertyRenderer: function(b, c, a, d) {
            return d && d.dataIndex && a && a.data && a.data[this.getXSSPropertyName(d.dataIndex)] ? (b = a.data[this.getXSSPropertyName(d.dataIndex)],
            d.ellipsisLimit && !isNaN(d.ellipsisLimit) && (b = _g.Util.ellipsis(b, d.ellipsisLimit, !0)),
            b) : b ? b : ""
        }
    }
}
...
_g.shared.String = new function() {
    return {
        startsWith: function(b, c) {
            return null == b || null == c ? null == b && null == c : c.length > b.length ? !1 : 0 == b.toString().indexOf(c.toString())
        },
...
_g.shared.ClientSidePersistence = function(b) {
    var c = {
        ...
        getKeys: function() {
            ...
            if (a)
                for (var c in a)
                    this.config.container ? 0 == c.indexOf(this.config.container + "/") && (a = c.substring(this.config.container.length + 1),
                    b.push(a)) : b.push(c);
            return b
        },
        get: function(a) {
        ...
        set: function(a, b) {
            ...
            if (a.length) {
                ...
                b ? e[a] = encodeURIComponent(b) : (c.action = "remove",
                delete e[a]);
                for (var k in e)
                    d.push(k + ":\x3d" + e[k]);
                this.cache = e;
                ...
                _g.$(_g.shared.ClientSidePersistence).trigger(_g.shared.ClientSidePersistence.EVENT_NAME, c)
            }
        },
        getMap: function() {
...
_g.shared.ClientSidePersistence.MODE_COOKIE = {
    ...
    write: function(b, c) {
        if (!Granite.OptOutUtil.isOptedOut() || Granite.OptOutUtil.maySetCookie(this.COOKIE_NAME))
            c ? _g.shared.ClientSidePersistence.CookieHelper.set(this.COOKIE_NAME, c, 365) : _g.shared.ClientSidePersistence.CookieHelper.erase(this.COOKIE_NAME)
    }
};
...
_g.shared.ClientSidePersistence.CookieHelper = {
    ...
    read: function(b) {
        ...
        for (var c = document.cookie.split(";"), a = 0; a < c.length; a++) {
            ...
            if (0 == d.indexOf(b))
                return (b = d.substring(b.length, d.length)) ? decodeURIComponent(b) : null
        }
        return null
    },
    erase: function(b) {
...
_g.shared.ClientSidePersistence.clearAllMaps = function() {
    _g.$.each([_g.shared.ClientSidePersistence.MODE_COOKIE, _g.shared.ClientSidePersistence.MODE_LOCAL, _g.shared.ClientSidePersistence.MODE_SESSION, _g.shared.ClientSidePersistence.MODE_WINDOW], function(b, c) {
...
CQ.shared.Form = function() {
    ...
    var getLabelNodeTextContent = function(node) {
        ...
        var walkTree = function(node) {
            if (node.nodeType == 3)
                text += node.nodeValue;
            if (node.nodeName.toLowerCase() == "select" || node.nodeName.toLowerCase() == "input" || node.nodeName.toLowerCase() == "textarea" || node.nodeName.toLowerCase() == "button")
                return;
